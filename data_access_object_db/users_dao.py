import psycopg2
import data_access_object_db.data_connect
...
class users_queries:
    ...
    def get_user_by_yum_id(yum_id):
        return f"select * from t_user where yum_id = '{yum_id}';"

    @staticmethod
    def get_hierarchy_user_by_yum_id(yum_id):
        return f"""select u.vendor_tool , u.user_type , u.yum_id ,u.operational_role, u.user_primary_brand ,hd.* 
...
class users_queries_execute:
    def __init__(self):
        self.cursor = data_access_object_db.data_connect.connect(load_db_config(section='users_dev'))

    def get_user_by_yum_id(self, yum_id):
        query = users_queries.get_user_by_yum_id(yum_id.lower())
        self.cursor.execute(query)
        rows = self.cursor.fetchall()
        return rows

    def get_hierarchy_user_by_yum_id(self, yum_id):
        query = users_queries.get_hierarchy_user_by_yum_id(yum_id.lower())
        self.cursor.execute(query)
        rows = self.cursor.fetchall()
        return rows
